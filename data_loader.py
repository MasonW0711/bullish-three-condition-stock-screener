...
def normalize_yfinance_data(df: pd.DataFrame, stock_code: str) -> pd.DataFrame:
    ...
    if isinstance(normalized.columns, pd.MultiIndex):
        level_values = normalized.columns.get_level_values(-1)
        if stock_code in level_values:
...
def _download_candidate(symbol: str, start_date, end_date) -> pd.DataFrame:
    return yf.download(
        symbol,
        start=start_date,
        end=end_date,
...
def download_stock_data(
    ...
    for raw_code in _dedupe_preserve_order(stock_codes):
        resolved_frame = pd.DataFrame(columns=REQUIRED_OHLCV_COLUMNS)
        resolved_symbol = None

        for candidate in normalize_symbol(raw_code):
            try:
                raw_data = _download_candidate(candidate, start_date, end_date)
            except Exception:
                # Treat third-party transport failures as a single-symbol failure
                # so the rest of the screening run can continue.
                continue
